...
// Paths extracted from Apple_Music_Icon_RGB_lg_073120.svg (361×361 viewBox)
const AM_BG_PATH =
  'M360,112.61c0-4.3,0-8.6-0.02-12.9c-0.02-3.62-0.06-7.24-0.16-10.86c-0.21-7.89-0.68-15.84-2.08-23.64' +
...
const AM_GLYPH_PATH =
  'M254.5,55c-0.87,0.08-8.6,1.45-9.53,1.64l-107,21.59l-0.04,0.01c-2.79,0.59-4.98,1.58-6.67,3' +
...
function AppleMusicIcon({ size = 72 }: { size?: number }) {
  return (
    <Svg width={size} height={size} viewBox="0 0 361 361">
      <Defs>
        <LinearGradient id="amGrad" x1="180" y1="358.6" x2="180" y2="7.76" gradientUnits="userSpaceOnUse">
          <Stop offset="0" stopColor="#FA233B" />
          <Stop offset="1" stopColor="#FB5C74" />
        </LinearGradient>
      </Defs>
      <Path d={AM_BG_PATH} fill="url(#amGrad)" fillRule="evenodd" clipRule="evenodd" />
      <Path d={AM_GLYPH_PATH} fill="#FFFFFF" fillRule="evenodd" clipRule="evenodd" />
    </Svg>
  );
}

const BENEFITS = [
  'Import your recent listening history',
  'Rate tracks from your library',
  'Keep your music diary up to date',
] as const;

interface AppleMusicConnectPanelProps {
  title: string;
  description: string;
    ...
    <View style={{ alignItems: 'center', paddingHorizontal: 32 }}>
      ...
      {isConnecting ? (
        <View style={{ width: '100%', alignItems: 'center', gap: 14 }}>
          <ActivityIndicator color={APPLE_MUSIC_COLOR} size="large" />
          <Animated.Text
            key={loadingLabel}
        ...
        <>
          <Pressable
            ...
            style={({ pressed }) => ({
              ...
              backgroundColor: APPLE_MUSIC_COLOR,
              alignItems: 'center',
