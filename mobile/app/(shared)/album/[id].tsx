...
export default function AlbumScreen() {
  ...
  const { data: userReview } = useUserAlbumReview(id);
  ...
  function handleRateButtonPress() {
    ...
    router.push({ pathname: '/rate', params: { albumId: id, type: 'album' } });
  }

  function openShareSheet() {
    if (!album || !userRating || !user) return;

    const shareId = sharePreviewStore.set({
      variant: 'review',
      data: {
        songOrAlbumName: album.name,
  ...
  return (
    <>
      ...
      >
        ...
        {/* Glass circle share button — only shown when rated */}
